 ...
 *
 * <p>The default {@code @Bean} keeps every message. When the real rules land, dropped messages are
 * marked PROCESSED in the inbox with no outbox row written.
 */
@FunctionalInterface
public interface TradeFilter {

    /** {@code true} to keep (publish) the message, {@code false} to drop it. */
    boolean keep(Region region, String pivotPayload);
}

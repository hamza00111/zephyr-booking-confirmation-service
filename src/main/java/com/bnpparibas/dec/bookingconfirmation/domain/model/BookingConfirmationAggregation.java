 ...
 * the two differ (Created+Amended collapses into a TRADE_CREATED event carrying the amended payload). A
 ...
 */
public record BookingConfirmationAggregation(
        @Nullable ParsedTradeEvent survivor, @Nullable TradeEventType emitAs, List<Long> collapsedIds) {

    public static BookingConfirmationAggregation emit(
            final ParsedTradeEvent survivor, final TradeEventType emitAs, final List<Long> collapsedIds) {
        return new BookingConfirmationAggregation(survivor, emitAs, collapsedIds);
    }

    public static BookingConfirmationAggregation dropAll(final List<Long> collapsedIds) {
        return new BookingConfirmationAggregation(null, null, collapsedIds);
    }
}
